...
def run(
        ...
        data=ROOT / 'data/firbox.yaml',  # dataset.yaml path
        ...
        augment=False,  # augmented inferenceTTA测试时增强/多尺度预测
    ...
    '''====================推理部分======================='''
    # Run inference
    model.warmup(imgsz=(1 if pt else bs, 3, *imgsz))  # warmup 热身（空白实验）
    seen, windows, dt = 0, [], [0.0, 0.0, 0.0]# seen：已经处理完了多少图片， dt：每一步耗时
    for path, im, im0s, vid_cap, s in dataset:# 对数据集每张图片进行预处理
        t1 = time_sync()
        im = torch.from_numpy(im).to(device)
        im = im.half() if model.fp16 else im.float()  # uint8 to fp16/32
        im /= 255  # 0 - 255 to 0.0 - 1.0归一化的图片
        if len(im.shape) == 3:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # 将训练时放缩了的图片标注bounding box调整为与原图一致，将预测信息映射到原图
...
def main(opt):
    ...
    run(**vars(opt))# 将opt变量的属性和属性值作为关键字传递给run函数
